# Twin brief — payments-and-commerce (Wave 3)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/payments-and-commerce.html`.
Deliverables (ONLY inside `/tmp/twin-payments-and-commerce/`):
- `en.html` — copy of the EN file (start: `cp reference/payments-and-commerce.html /tmp/twin-payments-and-commerce/en.html`)
...
- visible h2s (NH): 21 | numbered h2s still in file: 0 | chars ≈ 44695
- §-tokens in body text (outside <pre> and existing links): 3×3, 5×2
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../payments-and-commerce.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-payments-and-commerce/en.html /tmp/twin-payments-and-commerce/ar.html 21 --map /tmp/twin-payments-and-commerce/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-payments-and-commerce/en.html /tmp/twin-payments-and-commerce/ar.html payments-and-commerce` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-payments-and-commerce/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-payments-and-commerce/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
