# Twin brief — recurring-fear-of-replacement (Wave 5)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/recurring-fear-of-replacement.html`.
Deliverables (ONLY inside `/tmp/twin-recurring-fear-of-replacement/`):
- `en.html` — copy of the EN file (start: `cp reference/recurring-fear-of-replacement.html /tmp/twin-recurring-fear-of-replacement/en.html`)
...
- visible h2s (NH): 21 | chars ≈ 37335
- §-tokens outside pre/links: §9c×2 §10×1
- no numbered h2s. §9c/§10 are legacy pointers — read the context; candidates: this doc's pre-restructure numbering or problem-solving/cs-and-se legacy. Title-exact content match wins.
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../recurring-fear-of-replacement.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-recurring-fear-of-replacement/en.html /tmp/twin-recurring-fear-of-replacement/ar.html 21 --map /tmp/twin-recurring-fear-of-replacement/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-recurring-fear-of-replacement/en.html /tmp/twin-recurring-fear-of-replacement/ar.html recurring-fear-of-replacement` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-recurring-fear-of-replacement/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-recurring-fear-of-replacement/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
