# Twin brief — terminal-and-deployment-substrate (Wave 1)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/terminal-and-deployment-substrate.html`.
Deliverables (ONLY inside `/tmp/twin-terminal-and-deployment-substrate/`):
- `en.html` — copy of the EN file (start: `cp reference/terminal-and-deployment-substrate.html /tmp/twin-terminal-and-deployment-substrate/en.html`)
...
- visible h2s (NH): 14 | numbered h2s still in file: 0 | chars ≈ 31755
- §-tokens in body text (outside <pre> and existing links): none
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../terminal-and-deployment-substrate.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-terminal-and-deployment-substrate/en.html /tmp/twin-terminal-and-deployment-substrate/ar.html 14 --map /tmp/twin-terminal-and-deployment-substrate/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-terminal-and-deployment-substrate/en.html /tmp/twin-terminal-and-deployment-substrate/ar.html terminal-and-deployment-substrate` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-terminal-and-deployment-substrate/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-terminal-and-deployment-substrate/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
