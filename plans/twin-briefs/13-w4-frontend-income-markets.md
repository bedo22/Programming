# Twin brief — frontend-income-markets (Wave 4)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/frontend-income-markets.html`.
Deliverables (ONLY inside `/tmp/twin-frontend-income-markets/`):
- `en.html` — copy of the EN file (start: `cp reference/frontend-income-markets.html /tmp/twin-frontend-income-markets/en.html`)
...
- visible h2s (NH): 10 | chars ≈ 15775
- §-tokens outside pre/links: §6×1 (target: section 6 «Education, content & community» — positional)
- numbered h2s 1–10 remain (de-numbered by the pipeline); §6 points at the doc's own section 6
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../frontend-income-markets.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-frontend-income-markets/en.html /tmp/twin-frontend-income-markets/ar.html 10 --map /tmp/twin-frontend-income-markets/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-frontend-income-markets/en.html /tmp/twin-frontend-income-markets/ar.html frontend-income-markets` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-frontend-income-markets/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-frontend-income-markets/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
