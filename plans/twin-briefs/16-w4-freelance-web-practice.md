# Twin brief — freelance-web-practice (Wave 4)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/freelance-web-practice.html`.
Deliverables (ONLY inside `/tmp/twin-freelance-web-practice/`):
- `en.html` — copy of the EN file (start: `cp reference/freelance-web-practice.html /tmp/twin-freelance-web-practice/en.html`)
...
- visible h2s (NH): 19 | chars ≈ 38505
- §-tokens outside pre/links: none
- no numbered h2s, zero §-tokens — cleanest TRANSLATE on the wave
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../freelance-web-practice.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-freelance-web-practice/en.html /tmp/twin-freelance-web-practice/ar.html 19 --map /tmp/twin-freelance-web-practice/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-freelance-web-practice/en.html /tmp/twin-freelance-web-practice/ar.html freelance-web-practice` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-freelance-web-practice/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-freelance-web-practice/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
