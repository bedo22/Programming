# Twin brief — security-and-threat-modeling (Wave 3)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/security-and-threat-modeling.html`.
Deliverables (ONLY inside `/tmp/twin-security-and-threat-modeling/`):
- `en.html` — copy of the EN file (start: `cp reference/security-and-threat-modeling.html /tmp/twin-security-and-threat-modeling/en.html`)
...
- visible h2s (NH): 21 | numbered h2s still in file: 0 | chars ≈ 46653
- §-tokens in body text (outside <pre> and existing links): 10×3, 1d×4, 1e×4, 5d×3, 7×1, 8h×4
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../security-and-threat-modeling.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-security-and-threat-modeling/en.html /tmp/twin-security-and-threat-modeling/ar.html 21 --map /tmp/twin-security-and-threat-modeling/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-security-and-threat-modeling/en.html /tmp/twin-security-and-threat-modeling/ar.html security-and-threat-modeling` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-security-and-threat-modeling/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-security-and-threat-modeling/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
