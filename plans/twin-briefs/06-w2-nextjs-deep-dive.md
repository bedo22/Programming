# Twin brief — nextjs-deep-dive (Wave 2)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/nextjs-deep-dive.html`.
Deliverables (ONLY inside `/tmp/twin-nextjs-deep-dive/`):
- `en.html` — copy of the EN file (start: `cp reference/nextjs-deep-dive.html /tmp/twin-nextjs-deep-dive/en.html`)
...
- visible h2s (NH): 26 | numbered h2s still in file: 26 | chars ≈ 48693
- §-tokens in body text (outside <pre> and existing links): 1×1, 10×2, 10.1×1, 11×1, 12×2, 13×1, 14×1, 15×5, 16.×1, 19×1, 1b×1, 1f×5, 2×4, 3×3, 4×5, 4.6×1, 5×6, 5.×1, 5.2×3, 5.3×5, 6×4, 6.1×1, 7×2, 7.1×1, 8×1, 9×3
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../nextjs-deep-dive.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-nextjs-deep-dive/en.html /tmp/twin-nextjs-deep-dive/ar.html 26 --map /tmp/twin-nextjs-deep-dive/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-nextjs-deep-dive/en.html /tmp/twin-nextjs-deep-dive/ar.html nextjs-deep-dive` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-nextjs-deep-dive/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-nextjs-deep-dive/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
