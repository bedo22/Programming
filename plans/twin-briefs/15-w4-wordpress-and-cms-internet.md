# Twin brief — wordpress-and-cms-internet (Wave 4)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/wordpress-and-cms-internet.html`.
Deliverables (ONLY inside `/tmp/twin-wordpress-and-cms-internet/`):
- `en.html` — copy of the EN file (start: `cp reference/wordpress-and-cms-internet.html /tmp/twin-wordpress-and-cms-internet/en.html`)
...
- visible h2s (NH): 21 | chars ≈ 58037
- §-tokens outside pre/links: §1×2 §7×1
- no numbered h2s; §1/§7 target the doc's own sections 1 (definition) and 7 (how-wordpress-works) — title-exact
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../wordpress-and-cms-internet.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-wordpress-and-cms-internet/en.html /tmp/twin-wordpress-and-cms-internet/ar.html 21 --map /tmp/twin-wordpress-and-cms-internet/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-wordpress-and-cms-internet/en.html /tmp/twin-wordpress-and-cms-internet/ar.html wordpress-and-cms-internet` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-wordpress-and-cms-internet/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-wordpress-and-cms-internet/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
