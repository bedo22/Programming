# Twin brief — product-shapes (Wave 5)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/product-shapes.html`.
Deliverables (ONLY inside `/tmp/twin-product-shapes/`):
- `en.html` — copy of the EN file (start: `cp reference/product-shapes.html /tmp/twin-product-shapes/en.html`)
...
- visible h2s (NH): 18 | chars ≈ 29372
- §-tokens outside pre/links: §3b×1 §6j×1 §7×1
- no numbered h2s. §3b/§6j look like html-and-css legacy lettered sections (3b = platform primitives, 6j = core web vitals — VERIFY by reading the target content, title-exact wins); §7 needs a context read.
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../product-shapes.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-product-shapes/en.html /tmp/twin-product-shapes/ar.html 18 --map /tmp/twin-product-shapes/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-product-shapes/en.html /tmp/twin-product-shapes/ar.html product-shapes` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-product-shapes/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-product-shapes/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
