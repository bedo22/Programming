# Twin brief — income-stream-landscape (Wave 6)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/income-stream-landscape.html`.
Deliverables (ONLY inside `/tmp/twin-income-stream-landscape/`):
- `en.html` — copy of the EN file (start: `cp reference/income-stream-landscape.html /tmp/twin-income-stream-landscape/en.html`)
...
- visible h2s (NH): 10 | chars ≈ 20961
- §-tokens outside pre/links: §3×1 §5×1 §6a×1 §6b×1 §7×1
- 9 numbered h2s (0–8, incl. «2. Live demand sample save changes» workbook labels) — de-numbered by pipeline. §6a/§6b → doc-internal into section 6 (Conversion log); §3/§5/§7 self-refs. NOTE: this is a WORKBOOK — sections like «Weekly demand sample» are worksheet pages; translate labels faithfully.
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../income-stream-landscape.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-income-stream-landscape/en.html /tmp/twin-income-stream-landscape/ar.html 10 --map /tmp/twin-income-stream-landscape/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-income-stream-landscape/en.html /tmp/twin-income-stream-landscape/ar.html income-stream-landscape` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-income-stream-landscape/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-income-stream-landscape/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
