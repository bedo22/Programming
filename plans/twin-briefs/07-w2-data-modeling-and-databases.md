# Twin brief — data-modeling-and-databases (Wave 2)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/data-modeling-and-databases.html`.
Deliverables (ONLY inside `/tmp/twin-data-modeling-and-databases/`):
- `en.html` — copy of the EN file (start: `cp reference/data-modeling-and-databases.html /tmp/twin-data-modeling-and-databases/en.html`)
...
- visible h2s (NH): 25 | numbered h2s still in file: 0 | chars ≈ 46458
- §-tokens in body text (outside <pre> and existing links): 1e×4, 5×4, 6b×1, 6c×2, 6e×1, 6f×1, 6g×1
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../data-modeling-and-databases.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-data-modeling-and-databases/en.html /tmp/twin-data-modeling-and-databases/ar.html 25 --map /tmp/twin-data-modeling-and-databases/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-data-modeling-and-databases/en.html /tmp/twin-data-modeling-and-databases/ar.html data-modeling-and-databases` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-data-modeling-and-databases/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-data-modeling-and-databases/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
