# Twin brief — beyond-the-browser (Wave 6)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/beyond-the-browser.html`.
Deliverables (ONLY inside `/tmp/twin-beyond-the-browser/`):
- `en.html` — copy of the EN file (start: `cp reference/beyond-the-browser.html /tmp/twin-beyond-the-browser/en.html`)
...
- visible h2s (NH): 18 | chars ≈ 25973
- §-tokens outside pre/links: §3×1 §3b×1 §4×7 §5×1 §6×2 §7×1 §8×1 §9×1 (15 total)
- no numbered h2s. §4×7 are heavy — likely self-refs to section 4 (verify by title-exact read); §3b/§5/§6/§7/§8/§9 same treatment.
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../beyond-the-browser.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-beyond-the-browser/en.html /tmp/twin-beyond-the-browser/ar.html 18 --map /tmp/twin-beyond-the-browser/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-beyond-the-browser/en.html /tmp/twin-beyond-the-browser/ar.html beyond-the-browser` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-beyond-the-browser/).
...
- Mojibake safety: verify with `python3 -c "open('/tmp/twin-beyond-the-browser/ar.html',encoding='utf-8').read()"` — never judge Arabic by terminal echo.
