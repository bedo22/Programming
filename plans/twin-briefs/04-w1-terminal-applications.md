# Twin brief — terminal-applications (Wave 1)

## Task (TRANSLATE mode — no AR twin exists)
Produce the full Arabic twin + a per-doc hygiene map for `reference/terminal-applications.html`.
Deliverables (ONLY inside `/tmp/twin-terminal-applications/`):
- `en.html` — copy of the EN file (start: `cp reference/terminal-applications.html /tmp/twin-terminal-applications/en.html`)
...
- visible h2s (NH): 18 | numbered h2s still in file: 0 | chars ≈ 25518
- §-tokens in body text (outside <pre> and existing links): 3b×1, 5b.×1
...
3. Write the whole AR twin in one pass: `lang="ar" dir="rtl"`, asset paths `../../assets/…`, lang-switch header line `../terminal-applications.html (English)`, h2 titles translated, **h2 ids = EN's ids verbatim** (mirror), tables row-for-row, `<pre>` blocks verbatim, `&amp;` escaping. Too big for one write → parts in /tmp, concatenate, size-check each part. Full translation (ratio band 0.75–0.97): do NOT abbreviate.
4. Hygiene on YOUR copies only:
   `python3 ~/.agents/skills/translate-to-arabic/scripts/twin-pipeline.py /tmp/twin-terminal-applications/en.html /tmp/twin-terminal-applications/ar.html 18 --map /tmp/twin-terminal-applications/map.json --strict-folds`
...
5. Gate: `python3 ~/.agents/skills/translate-to-arabic/scripts/verify-twins.py /tmp/twin-terminal-applications/en.html /tmp/twin-terminal-applications/ar.html terminal-applications` → target ALL GATES PASSED; an out-of-band ratio is a recorded-exception flag → note density reasoning in report.md.
6. report.md last, then STOP (no further work, no cleanup of /tmp/twin-terminal-applications/).
