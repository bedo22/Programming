...
script supplies the QUOTE TEXT by slicing the transcript itself, so a quote can never be
...
usage: build_meh.py <session-key> <agent-json> <out-yaml>      e.g.  ts-004
"""
...
def resolve_transcript(key: str) -> pathlib.Path:
...
def main():
    key, aj, out = sys.argv[1], sys.argv[2], sys.argv[3]
