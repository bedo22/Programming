...
# W4.5: root from find_root() — cwd-preferred, like the sibling gates. The
# __file__ walk resolved the skill tree and produced the reproduced 0/0-green
# ('DOCS FLAGGED: 0/0', exit 0) on a corpus it could not see.
import _shelf_lib as _lib
from shelf_core.notes import claims_count as _claims_count  # noqa: E402 (A5.3(d))

ROOT = str(_lib.find_root())
...
def load_cfg():
    ...
    cfg = dict(DEFAULTS)
    cfg.update(_lib.gates_cfg())
    return cfg


CFG = load_cfg()
_CORPUS = _lib.corpus_cfg()
_TD = str((_CORPUS or {}).get("transcripts_dir") or "transcripts/clean")
NOTES = str(_lib.notes_dir(ROOT))
...
# 2026-08-24) encodes measured semantics the generic engine above does not
...
def _fork_main(argv):
    ...
    if not all_docs() and not argv:
        print("no docs under %s (root %s) — nothing measured" % (DOCS_DIR, ROOT),
              file=sys.stderr)
        return 2
    paths = [os.path.abspath(p) for p in argv] or all_docs()
    main_note_quotes = _fork_make_note_quotes(notes_dir, "")
    fail, lines, triage_all = _fork_analyze(all_docs(), paths,
                                            main_note_quotes, session_max,
                                            kw, cfg)
    for l in lines:
        print(l)
    print("\nDOCS FLAGGED (hard gate): %d/%d" % (fail, len(paths)))
    if triage_all:
...
if __name__ == "__main__":
    if CFG.get("coverage_profile") == "quotes-responsibility":
        sys.exit(_fork_main(sys.argv[1:]))
